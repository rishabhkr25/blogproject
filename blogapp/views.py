...
class PostListView(ListView):
    ...
    def get_queryset(self):
        return Post.objects.filter(piublished_date__lte=timezone.now()).order_by('-published_date')
...
class PostUpdateView(LoginRequiredMixin, UpdateView):
    login_url = '/login/'
    redirect_field_name = 'blogapp/post_details.html'
    form_class = PostForm
    model = Post
...
def comment_remove(request,pk):
    ...
    comment.delete()
    return redirect('posy_detail', pk=post_pk)